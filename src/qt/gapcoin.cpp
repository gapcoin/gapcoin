...
{
    return QCoreApplication::translate("gapcoin-core", psz).toStdString();
}
...
void GapcoinApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        // gapcoin: URIs or payment requests:
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data directory and parse gapcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!boost::filesystem::is_directory(GetDataDir(false)))
    {
        QMessageBox::critical(0, QObject::tr("Gapcoin"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Gapcoin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // gapcoin: links repeatedly have their payment requests routed to this process:
    ...
    try
    {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Gapcoin Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
