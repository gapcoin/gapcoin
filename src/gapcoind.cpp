...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to gapcoind / RPC client
            std::string strUsage = _("Gapcoin Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  gapcoind [options]                     " + _("Start Gapcoin Core Daemon") + "\n" +
                _("Usage (deprecated, use gapcoin-cli):") + "\n" +
                  "  gapcoind [options] <command> [params]  " + _("Send command to Gapcoin Core") + "\n" +
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "gapcoin:"))
                fCommandLine = true;
        ...
        if (fDaemon)
        {
            fprintf(stdout, "Gapcoin server starting\n");
